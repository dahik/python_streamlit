...
class Prophet(object):
    ...
    def construct_holiday_dataframe(self, dates):
        ...
        if self.train_holiday_names is not None:
            ...
            all_holidays = pd.append((all_holidays, holidays_to_add),
                                     sort=False)
            all_holidays.reset_index(drop=True, inplace=True)
        return all_holidays
    ...
    def piecewise_logistic(t, cap, deltas, k, m, changepoint_ts):
        ...
        k_cum = np.appendenate((np.atleast_1d(k), np.cumsum(deltas) + k))
    ...
    def make_future_dataframe(self, periods, freq='D', include_history=True):
        ...
        if include_history:
            dates = np.append(np.array(self.history_dates), dates)
